...
目标平台没有 libc，所以有两处要自己补：
  - firmware/freestanding/math.h：common/pid.c 与 firmware/brew.c 都
    #include <math.h>，其中真正用到的 libm 函数只有 fabsf，这个头声明它；
  - firmware/libc_min.c：实现 fabsf 与 __libc_init_array（启动文件要用），
    以及编译器可能内联生成调用的 memcpy/memset 等。
另外 firmware/freestanding/stm32f10x.h 是一个兼容转发头：源文件里写的是
老 SPL 时代的 "stm32f10x.h"，这里把它转到 CMSIS 的 "stm32f1xx.h"，
这样既不用改源文件、也不必往仓库里塞一份停止维护的老库。
...
CPU = "cortex_m3"          # 注意：下划线，不能写 cortex-m3

# 关于浮点：和 SmartGarden 那边（全程整数、没有一处 float）不同，
# 本固件**确实在用浮点** —— PID、温度换算、占空比都是 float。
# 这里仍然不传 -mfpu / -mfloat-abi，走软浮点 ABI，理由：
#   1) STM32F103 是 Cortex-M3，核心里根本没有 FPU，谈不上"用不用"；
#   2) 软浮点下 fadd/fmul 会落到 __aeabi_fadd / __aeabi_fmul，
#      这些函数由 zig 自带的 compiler-rt 提供。链接时用的是
#      -nostartfiles 而不是 -nostdlib，正是为了把它保留下来，
#      所以不需要 libm、也不需要额外补运行时；
#   3) 不存在"ELF 头标 hard-float 而代码是软浮点"的不一致问题
#      ——那个坑只在带 VFP 的 M4/M7 上才会踩到。
DEFINES = [
    "STM32F103xB",
]
...
PROJECT_SRC = [
    ...
    "firmware/protection.c",
    "firmware/board_stm32f1.c",
    "firmware/libc_min.c",
    "firmware/faults.c",
]

# 算法核心：不依赖任何 STM32 头文件，与板级实现完全解耦
COMMON_SRC = [
    "common/filters.c",
    "common/pid.c",
]
...
# 没有 HAL、也没有 RTOS：本工程是直写寄存器的裸机前后台结构，
# 只从 CMSIS 拿设备头（寄存器定义）和启动/时钟文件。
CMSIS_SRC = [
    "third_party/cmsis-device-f1/system_stm32f1xx.c",
]

STARTUP = "third_party/cmsis-device-f1/startup_stm32f103xb.s"

LINKER_SCRIPT = "firmware/stm32f103_flash.ld"

# ---------------------------------------------------------------- include 路径

# firmware/freestanding 必须排在前面：它提供 freestanding 构建缺失的 <math.h>
INCLUDES = [
    "firmware",
    "firmware/freestanding",
    "common",
    "third_party/cmsis-core",
    "third_party/cmsis-device-f1/Include",
]
...
MEM_REGIONS = [
    # 按 STM32F103C8T6（Blue Pill 那一类）算：64KB Flash / 20KB SRAM。
    # 仓库里的代码没有依赖更大容量的东西，按最小号设可以把"将来改崩了
    # 放不下"这件事在本地就暴露出来，而不是等到上板才炸。
    ("FLASH", 0x08000000, 64 * 1024),
    ("RAM", 0x20000000, 20 * 1024),
]
...
def main():
    ...
    missing = [p for p in (CMSIS_SRC + [STARTUP]) if not (ROOT / p).exists()]
    if missing:
    ...
    sources = PROJECT_SRC + COMMON_SRC + CMSIS_SRC
    objs = []
    for src in sources:
        o = obj_path(src)
        objs.append(o)
    ...
    # 启动文件（汇编）单独编
    startup_obj = BUILD / "startup_stm32f103xb.o"
    ...
    # ---------------- 链接 ----------------
    elf = BUILD / "smartcoffeemachine.elf"
    symf = BUILD / "smartcoffeemachine.sym.txt"
    link = [
        "cc",
        "-target", TARGET,
        "-mcpu=" + CPU,
    ...
    # ---------------- 产出烧写格式 ----------------
    binf = BUILD / "smartcoffeemachine.bin"
    hexf = BUILD / "smartcoffeemachine.hex"
    ...
    print("烧写（OpenOCD）：openocd -f interface/stlink.cfg -f target/stm32f1x.cfg \\")
